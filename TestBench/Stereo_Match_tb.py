...
async def test_stereo_dataset(dut):
    ...
    dut.i_rst.value=0
    dut.i_dato_L.value=0
    dut.i_dato_R.value=0
    dut.i_dval.value=0
    dut.i_Tresh_LRCC.value=8
    ...
    with open("../output_vector_data.txt","w") as fp_data, open("../output_vector_valid.txt","w") as fp_valid:
        ...
        #for i in range(500):
            dut.i_dato_L.value=int(Vector_left[i])
            dut.i_dato_R.value=int(Vector_right[i])
            dut.i_dval.value=int(Vector_valid[i])
            await FallingEdge(dut.i_clk)
            fp_data.write(f"{int(dut.o_dato.value)}\n")
